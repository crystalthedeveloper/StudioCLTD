...
frame();assert.equal(active(trail),0);assert.equal(active(puffs),0,'stop clears immediately');
...
console.log('Rolling effects passed: radius/distance rotation, ramps, rest, teleport, contact shadow, movement emission, airborne suppression, landing burst and immediate stop/pause cleanup.');
